...
public class MemberApiController implements MemberApiSpecification{
    ...
    @PreAuthorize("isAuthenticated()")
    public ResponseEntity<ApiPageResponseSpec<List<PageResponse>>> memberDeletionProcess(PrincipalDetails principalDetails){

        ApiPageResponseSpec<List<PageResponse>> pagesWithUniqueHost= memberService.processMemberDeletion(principalDetails);
        return ResponseEntity.ok()
